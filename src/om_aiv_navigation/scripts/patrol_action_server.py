...
from om_aiv_navigation.msg import PatrolAction, PatrolFeedback, PatrolResult
import math
import numpy as np
import socket
import threading
...
class ActionServer():
    ...
    def __init__(self):
        self.a_server = actionlib.SimpleActionServer(
            "patrol", PatrolAction, execute_cb=self.execute_cb, auto_start=False)
        self.a_server.start()
    ...
    def execute_cb(self, goal):
        ...
        global rcv
        global i
        pub = rospy.Publisher('ldarcl_patrol', String, queue_size=10)
        rate = rospy.Rate(10) # 10hz
        print(Style.RESET_ALL)
        print(Fore.GREEN)

        #send command to arcl
        command = "patrol {}".format(task)
